...
class Parser extends JavaTokenParsers {
  ...
  def if_instr: Parser[Node] =
    "if" ~> expression ~ (":" ~> suite) ^^ { case expr ~ suite => IfInstr(expr, suite) }

  def elif_instr: Parser[Node] =
    "elif" ~> expression ~ (":" ~> suite) ^^ { case expr ~ suite => ElifInstr(expr, suite) }

  def if_else_stmt: Parser[Node] =
    if_instr ~ rep(elif_instr) ~ ("else" ~ ":" ~> suite).? ^^ {
      case instr ~ instrList ~ Some(suite) => IfElseInstr(instr :: instrList, suite)
      case instr ~ instrList ~ None => IfElifInstr(instr :: instrList)
  }
